...
> **企业级制造执行系统**，专为现代制造业打造的数字化生产管理平台

本项目是一套**功能完善、架构先进**的制造执行系统(MES)，致力于实现生产过程的**全流程数字化管理**。系统采用**企业级三层架构设计**，具备卓越的可维护性、可扩展性和高性能表现，为制造企业提供从生产计划到产品交付的**完整解决方案**。
...
| 📖 文档类型 | 📄 文档名称 | 📝 描述 |
|-------------|-------------|---------|
| **🚀 快速上手** | [开发指南](docs/开发指南.md) | 完整的开发环境配置和工作流程 |
| **🏗️ 系统设计** | [系统架构](docs/系统架构.md) | 详细的技术架构和模块设计 |
| **🔄 协作流程** | [Git工作流程](docs/Git工作流程.md) | 团队协作和版本控制规范 |
| **📊 项目管理** | [项目总览](docs/项目总览.md) | 实时项目进度和里程碑记录 |
| **🛠️ 运维部署** | [部署运维](docs/部署运维.md) | 系统部署、配置和运维指南 |
...
- **🔄 简化Git工作流** - 仅使用 `main` 和 `develop` 两个分支
- **⚡ 高效协作** - 团队成员直接在 `develop` 分支协作开发
- **🛡️ 质量保障** - 天帝负责 `develop` → `main` 的稳定版本发布
- **📊 实时同步** - 所有成员基于最新 `develop` 分支进行开发
