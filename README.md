...
- **日志可观测**：命令面板支持打开日志目录/今日日志/复制今日日志尾部（默认脱敏）/复制健康检查摘要（默认脱敏）/崩溃报告目录/最新崩溃报告/最新诊断包（zip），便于排障与回溯
...
- 崩溃报告会附带今日日志尾部上下文（已脱敏），便于快速定位问题发生前后的线索
...
- 目录回退：当日志目录不可创建/不可写时，会自动回退到 `%LocalAppData%\\MES-SON\\Logs`（避免安装目录权限问题）
...
- **Log Observability**: open log folder / today's log / copy today's log tail (masked by default) / copy health check summary (masked by default) / CrashReports folder / latest CrashReport / latest Support Bundle (zip, redacted by default) directly from the command palette
...
- Crash reports include a masked recent log tail snippet for faster triage
