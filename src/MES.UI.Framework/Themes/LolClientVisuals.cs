...
namespace MES.UI.Framework.Themes
{
    ...
    public static class LolClientVisuals
    {
            ...
            Font subtitleFont)
        {
            ...
            // 背景（必须每次都填充：按钮禁用了 OnPaintBackground，否则会出现残影/文字叠影）
            Color bgTop;
            Color bgBottom;

            if (isSelected || isHovered)
            {
                bgTop = isSelected ? LeagueColors.DarkSurfaceLight : LeagueColors.DarkSurface;
                bgBottom = LeagueColors.DarkPanel;
            }
            else
            {
                // 普通态更贴近侧边栏，但仍保持“可见的暗底块”，避免透明叠影
                bgTop = LeagueColors.DarkestBackground;
                bgBottom = LeagueColors.DarkBackground;
            }
            ...
            // 边框（悬停更亮）
            var borderColor = isHovered ? LeagueColors.RiotGoldHover : Color.FromArgb(120, LeagueColors.RiotBorderGold);
            ...
            {
                var iconColor = isSelected ? LeagueColors.RiotGoldHover : LeagueColors.TextPrimary;
                if (isHovered) iconColor = LeagueColors.TextHighlight;
            ...
            // Title
            var titleColor = isSelected ? LeagueColors.TextHighlight : LeagueColors.TextPrimary;
            if (isHovered) titleColor = LeagueColors.TextHighlight;

            using (var titleBrush = new SolidBrush(titleColor))
            using (var subtitleBrush = new SolidBrush(LeagueColors.TextSecondary))
            {
            ...
            Font bodyFont)
        {
            ...
            // 背景渐变
            var bgTop = LeagueColors.DarkSurface;
            var bgBottom = LeagueColors.DarkPanel;
            using (var bgBrush = new LinearGradientBrush(drawRect, bgTop, bgBottom, LinearGradientMode.Vertical))
            {
                g.FillRectangle(bgBrush, drawRect);
            }
            ...
            {
                g.FillRectangle(overlay, drawRect);
            }

            // 悬停外发光（微弱）
            if (isHovered)
            {
                using (var glowPen = new Pen(Color.FromArgb(120, accentColor), 2))
                {
            ...
            // 内描边（更精致）
            var innerRect = drawRect;
            innerRect.Inflate(-3, -3);
            using (var innerPen = new Pen(Color.FromArgb(40, LeagueColors.RiotGoldHover), 1))
            {
                g.DrawRectangle(innerPen, innerRect);
            }

            // 顶部强调线
            using (var brush = new SolidBrush(Color.FromArgb(120, accentColor)))
            {
                g.FillRectangle(brush, drawRect.Left + 1, drawRect.Top + 1, drawRect.Width - 2, 2);
            }
            ...
            if (!string.IsNullOrWhiteSpace(iconGlyph))
            {
                ...
                if (watermarkFont != null)
                {
                    ...
                    {
                        using (watermarkFont)
                        using (var watermarkBrush = new SolidBrush(Color.FromArgb(18, accentColor)))
                        {
            ...
            using (var titleBrush = new SolidBrush(LeagueColors.TextHighlight))
            using (var bodyBrush = new SolidBrush(LeagueColors.TextSecondary))
            {
            ...
            // CTA（右下角“进入”）
            var ctaText = isHovered ? "进入  →" : "进入";
            using (var ctaBrush = new SolidBrush(Color.FromArgb(220, accentColor)))
