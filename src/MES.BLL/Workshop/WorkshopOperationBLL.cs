...
namespace MES.BLL.Workshop
{
    ...
    public class WorkshopOperationBLL : IWorkshopOperationBLL
    {
        ...
        /// <returns>操作是否成功</returns>
        public bool UpdateOperationStatus(string operationId, int status, DateTime? startTime)
        {
            try
            {
                var operation = GetOperationByOperationId(operationId);
                if (operation == null)
                {
                    LogManager.Error(string.Format("更新作业状态失败：作业编号 {0} 不存在", operationId));
                    return false;
                }

                operation.Status = status;
                operation.StatusText = GetStatusText(status);
                operation.UpdateTime = DateTime.Now;

                if (startTime.HasValue)
                {
                    operation.StartTime = startTime.Value;
                }

                bool result = UpdateOperation(operation);
                if (result)
                {
                    LogManager.Info(string.Format("成功更新作业状态：{0} -> {1}", operationId, operation.StatusText));
                }
            ...
            {
                LogManager.Error(string.Format("更新作业状态异常：{0}", ex.Message), ex);
                throw new MESException("更新作业状态时发生异常", ex);
            }
        }
