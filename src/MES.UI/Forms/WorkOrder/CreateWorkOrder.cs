    ...
    public partial class CreateWorkOrder : Form
    {
        ...
        private void SelectBOMDialog()
        {
            try
            {
                // 这里应该打开BOM选择窗体
                // 暂时使用简单的确认对话框模拟
                if (MessageBox.Show("是否加载默认BOM数据？", "选择BOM",
                    MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    currentBOMCode = "BOM001";
                    currentBOMVersion = "V1.0";
                    txtBOMCode.Text = currentBOMCode;
                    txtBOMVersion.Text = currentBOMVersion;
        ...
        private void AddBOMItem()
        {
            try
            {
                ...
                // 暂时添加模拟数据
                DataRow newRow = bomDataTable.NewRow();
                newRow["物料编号"] = string.Format("MAT{0:D3}", bomDataTable.Rows.Count + 1);
                newRow["物料名称"] = "新物料";
                newRow["规格型号"] = "标准";
                newRow["需求数量"] = 1;
                newRow["单位"] = "个";
        ...
        private void LoadSampleBOMData()
        {
            try
            {
                ...
                // 添加示例数据
                string[] materials = { "电阻", "电容", "芯片", "PCB板", "外壳" };
                string[] specs = { "1K欧", "100uF", "STM32", "FR4", "塑料" };
                decimal[] quantities = { 10, 5, 1, 1, 1 };
                string[] units = { "个", "个", "个", "片", "个" };

                for (int i = 0; i < materials.Length; i++)
                {
                    DataRow newRow = bomDataTable.NewRow();
                    newRow["物料编号"] = string.Format("MAT{0:D3}", i + 1);
                    newRow["物料名称"] = materials[i];
                    newRow["规格型号"] = specs[i];
                    newRow["需求数量"] = quantities[i];
                    newRow["单位"] = units[i];
                    newRow["库存数量"] = GetStockQuantity(string.Format("MAT{0:D3}", i + 1));
                    newRow["状态"] = GetMaterialStatus(string.Format("MAT{0:D3}", i + 1));
                    bomDataTable.Rows.Add(newRow);
                }
            ...
            {
                MessageBox.Show(string.Format("加载示例BOM数据失败：{0}", ex.Message),
                    "数据加载错误", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
