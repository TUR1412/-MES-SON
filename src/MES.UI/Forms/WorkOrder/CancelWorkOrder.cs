    ...
    public partial class CancelWorkOrder : Form
    {
        ...
        private void InitializeBusinessLogic()
        {
            try
            {
                ...
                LogManager.Info("取消工单窗体业务逻辑初始化完成");
            }
            catch (Exception ex)
            {
                LogManager.Error("取消工单窗体业务逻辑初始化失败", ex);
        ...
        private void InitializeUI()
        {
            try
            {
                ...
                LogManager.Info("取消工单窗体UI初始化完成");
            }
            catch (Exception ex)
            {
                LogManager.Error("取消工单窗体UI初始化失败", ex);
        ...
        private void dgvWorkOrders_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (dgvWorkOrders.SelectedRows.Count > 0)
                {
                    ...
                    // 启用取消按钮
                    btnCancel.Enabled = !string.IsNullOrEmpty(selectedWorkOrderNo);
                }
                else
                {
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var result = MessageBox.Show(
                    string.Format("确认要取消工单 [{0}] 吗？\n\n取消原因：{1}", selectedWorkOrderNo, txtCancelReason.Text),
                    "确认取消", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                if (result == DialogResult.Yes)
                {
                    ExecuteCancelWorkOrder();
                }
            }
            catch (Exception ex)
            {
                LogManager.Error("取消工单按钮点击处理失败", ex);
        ...
        /// 执行取消工单操作
        /// </summary>
        private void ExecuteCancelWorkOrder()
        {
            try
            {
                this.Cursor = Cursors.WaitCursor;
                ...
                {
                    MessageBox.Show("工单取消失败，请检查工单状态或联系系统管理员！", "失败",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
                LogManager.Error("执行取消工单操作失败", ex);
                MessageBox.Show(string.Format("取消工单失败：{0}", ex.Message), "错误",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
        ...
        /// 窗体加载事件
        /// </summary>
        private void CancelWorkOrder_Load(object sender, EventArgs e)
        {
            try
            {
                LogManager.Info("取消工单窗体加载完成");
            }
            catch (Exception ex)
            {
                LogManager.Error("取消工单窗体加载失败", ex);
            }
        }
        ...
        /// 窗体关闭事件
        /// </summary>
        private void CancelWorkOrder_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                LogManager.Info("取消工单窗体已关闭");
            }
            catch (Exception ex)
            {
                LogManager.Error("取消工单窗体关闭处理失败", ex);
            }
        }
