    ...
    public partial class ProcessRouteEditForm : Form
    {
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                OperationResult<ProcessRoute> result;

                if (_isEditMode)
                {
                    processRoute.Id = _processRoute.Id;
                    result = _processRouteService.UpdateProcessRoute(processRoute);
                }
                else
                {
                    result = _processRouteService.CreateProcessRoute(processRoute);
                }

                if (result.IsSuccess)
                {
                    _processRoute = result.Data;
                    IsSuccess = true;
                ...
                {
                    MessageBox.Show(string.Format("保存失败：{0}", result.Message), "错误",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
