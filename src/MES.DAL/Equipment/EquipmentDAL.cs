    ...
    public class EquipmentDAL : BaseDAL<EquipmentInfo>
    {
        ...
        protected override EquipmentInfo MapRowToEntity(DataRow row)
        {
            return new EquipmentInfo
            {
                ...
                EquipmentCode = row["equipment_code"]?.ToString() ?? string.Empty,
                EquipmentName = row["equipment_name"]?.ToString() ?? string.Empty,
                EquipmentType = row["equipment_type"]?.ToString() ?? string.Empty,
                WorkshopId = row["workshop_id"] != DBNull.Value ? Convert.ToInt32(row["workshop_id"]) : 0,
                WorkshopName = row["workshop_name"]?.ToString() ?? string.Empty,
                Status = Convert.ToInt32(row["status"]),
                Specification = row["specification"]?.ToString() ?? string.Empty,
                ...
                ResponsiblePersonName = row["responsible_person_name"]?.ToString() ?? string.Empty,
