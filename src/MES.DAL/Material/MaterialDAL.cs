...
namespace MES.DAL.Material
{
    ...
    public class MaterialDAL : BaseDAL<MaterialInfo>
    {
        ...
        public bool PhysicalDelete(int id)
        {
            try
            {
                string sql = "DELETE FROM material_info WHERE id = @id";
                var parameters = new[]
                {
                ...
                if (success)
                {
                    LogManager.Info(string.Format("物理删除MaterialInfo成功，ID: {0}", id));
                }
            ...
            {
                LogManager.Error(string.Format("物理删除MaterialInfo失败，ID: {0}", id), ex);
                throw new MESException("物理删除物料失败", ex);
            }
        }
