...
- 故障排查中心优化：跟随尾部刷新改为仅文件变更（LastWriteTime/Length）时读取，减少后台 IO
- 故障排查中心增强：补齐快捷键（Ctrl+F 聚焦搜索、F3/Shift+F3 Next/Prev、Ctrl+G 最新错误）并在元信息栏显示跟随状态/刷新间隔
- 命令面板增强：新增“打开崩溃报告目录/打开最新崩溃报告/打开最新诊断包（zip）”快捷入口
- 命令面板增强：新增“复制今日日志尾部到剪贴板”快捷入口
- 命令面板增强：新增“复制健康检查摘要”（快速/不测数据库；默认脱敏）快捷入口；“复制今日日志尾部”默认脱敏
...
- 仓库清理：移除误提交的 `*.backup` 文件，并加入 `.gitignore` 忽略规则
